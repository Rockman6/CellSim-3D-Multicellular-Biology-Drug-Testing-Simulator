...
is derived from \file{/Users/henry/plans/make-this-plan-into-virtual-key.md}
sections \S54--\S62.
...
\begin{itemize}
  ...
  \item Phase G6 (validation harness \texttt{cellsim\_blind\_benchmarks}
        consolidates the biology validators into one pass-rate test.
        6 literature-traceable benchmarks in the initial harness:
        BM01 vehicle control (Sorenson 1990),
        BM02 sub-threshold damage non-activation (Lahav 2004, Rothkamm 2003),
        BM03 mid-dose cisplatin 24-h G2/M arrest (Sorenson 1990, Velma 2016),
        BM04 mid-dose cisplatin 72-h BH3-only/BAX commitment
              (Yu 2001, Zeng 2019),
        BM05 Purvis 2012 p53 oscillation (Purvis \& Lahav 2012),
        BM06 high-dose cisplatin 72-h saturated MOMP (Cepeda 2007).
        \textbf{All 6 benchmarks pass across 5 pinned seeds} (30 / 30
        pass-rate). This is the first unified pass-rate metric in
        CellSim history — a pharmacology reviewer's "how many
        published phenotypes does your model reproduce?" test.
        CI-ready; any future PR that drops the pass-rate is rejected
        before merge. 2026-04-20).
  \item Phase G5 (\texttt{ApoTriggers.p53\_active} re-routed from
        raw \code{damageLevel} to the real \code{p53\_protein} pool
        via \code{linMap}$(0.12, 0.35)$; p53 pulses now drive the
        existing 11-trigger apoptosis cascade through PUMA $\to$ BAX
        $\to$ MOMP $\to$ caspase commitment;
        \texttt{cellsim\_cisplatin\_apoptosis} validator with 5 gates:
        control PUMA $\leq 1.0$,
        cisplatin PUMA $\geq 3.0$,
        PUMA ratio drug/ctl $\geq 5\times$,
        engine p53\_active $\geq 0.55$,
        Bax ratio drug/ctl $\geq 1.3\times$.
        Measured at damage 0.35 (physiologic cisplatin-equivalent),
        seed 0x1: PUMA ratio $20.7\times$, Bax ratio $1.79\times$,
        engine p53\_active 0.746.
        All 5 gates pass across 5 pinned seeds.
        Scope footer: the cell-kill execution step is gated by
        XIAP/Smac balance in the pre-existing cascade (caspase-3 stays
        at $\approx 0.04$ under saturated MOMP); that calibration is
        its own PR and does not change G5's commitment-signature claim.
        Zero seq02 drift, 2026-04-20).
  \item Phase G4 (p53 → p21 via CDKN1A promoter Hill(n=8) replaces
        the prior \texttt{damageLevel → p21} hack; new
        \texttt{cellsim\_cisplatin\_arrest} validator with 4 gates:
        drug p53 $\geq 1.25\times$ control (ATM axis active),
        drug G2+M $\geq 1.20\times$ control (cisplatin G2/M arrest,
        Sorenson 1990),
        drug divisions $\leq 0.85\times$ control (cytostatic),
        control divides $\geq 1\times$/cell/24 h (healthy baseline).
        Measured at seed 0x1: p53 ratio 1.79, G2+M ratio 2.35 (100\%
        cisplatin-G2 vs 42\% control), division ratio 0.124.
        All 4 gates pass across 4 pinned seeds.
        Calibration shift documented: seq02 CTC-HeLa error band
        moves from pre-G4 5.1--7.4\% to post-G4 4.9--7.3\%
        (mean $\approx 6.4 \pm 0.9\%$); the shift is within the
        pre-existing seed distribution, 2026-04-20).
